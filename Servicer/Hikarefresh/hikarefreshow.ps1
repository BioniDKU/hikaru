...
function Start-DownloadLoop($file) {
	while ($true) {
		Start-Process $env:SYSTEMDRIVE\Bionic\Hikarefresh\wget.exe -Wait -NoNewWindow -ArgumentList "https://github.com/BioniDKU/hikaru/releases/latest/download/${file}" -WorkingDirectory "$PSScriptRoot\Delivery"
		Show-WindowTitle 1
...
function Start-Hikarefreshing($hv) {
	...
	if (Test-Path -Path "$PSScriptRoot\Delivery\Resources.7z.old") {Remove-Item -Path "$PSScriptRoot\Delivery\Resources.7z.old" -Force}
	...
	if (Test-Path -Path "$PSScriptRoot\Delivery\Resources.7z") {Rename-Item -Path "$PSScriptRoot\Delivery\Resources.7z" -NewName Resources.7z.old}
	...
	if ($hv -eq 1) {
		Start-DownloadLoop "Resources.7z"
		while ($true) {
			try {
